...
pending snapshotを中間transaction後の`EditorState`へ引き継ぐ最初の実装では、foldの振れ幅は0pxになったが、unfoldは72pxのまま残った。

実Obsidianへ一時的なtransaction診断を入れた結果、カーソルがfold内にあるunfoldでは、同一selectionを再設定するtransaction自体がfolded rangeを解除し、そのtransactionに明示的な`unfoldEffect`が含まれないことが分かった。
...
さらにCodeMirrorの`foldState`は、明示的なselectionを持つtransactionのselection headがfolded range内にあると、そのrangeを自動的に解除する。

このimplicit unfoldには`unfoldEffect`がないため、明示的なfold/unfold effectだけを消費条件にすると、snapshotを画面が動いた後のstateへ引き継いでしまう。

## 目標

`click` capture後、native foldより前にselectionを含む中間transactionが入っても、補正済みsnapshotを最初にfolded rangeを変更するtransactionへ追加する。
...
transaction extenderがpending objectを見つけた場合、transactionのeffectとfolded rangeの実状態に応じて次のように扱う。

- `foldEffect`または`unfoldEffect`を含む場合は、snapshotを同じtransactionへ追加してpending objectを消費する。
- 明示的なeffectがなくても、`foldedRanges(transaction.startState)`と`foldedRanges(transaction.state)`を`RangeSet.eq()`で比較し、実内容が変わる場合はimplicit fold/unfoldとして同じtransactionでsnapshotを消費する。
- effectもfolded rangeの変化もない場合だけ、mappingを`transaction.startState`から`transaction.state`へ移し、同じpending objectを後続stateへ引き継ぐ。

これにより、selection transactionが単なる中間stateになる場合と、そのtransaction自体がimplicit unfoldになる場合の両方で、folded rangeを変更する最初のtransactionへsnapshotが届く。
...
fold内へカーソルを置いたstateを明示的な`foldEffect`で閉じ、同じselectionを再設定するtransactionだけでfolded rangeが1個から0個へ変わるcaseも追加する。

このcaseではselection transactionに`unfoldEffect`がなくても、補正済みsnapshotが同じtransactionへ含まれることを確認する。
...
実装後の再検証では、中間selection transactionあり・なしのfoldとunfold計4操作すべてで36 frameを記録し、見出しY座標と`scrollTop`の振れ幅がともに0px、selection headが344のまま、fold状態が反転することを確認した。
...
同一selection transactionが明示的な`unfoldEffect`なしでfolded rangeを解除する場合は、effectではなく`foldedRanges()`の実内容変化を消費条件に含めることも記録する。
